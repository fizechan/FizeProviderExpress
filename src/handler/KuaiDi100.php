...
namespace fize\provider\express\handler;


use fize\crypt\Json;
use fize\net\Http;
use fize\provider\express\ExpressHandler;

/**
 * 快递100
 */
class KuaiDi100 extends ExpressHandler
{
     ...
     * @return bool
     */
    public function query($num, $company = null)
    {
        $customer = $this->config['customer'];
        $key = $this->config['key'];
        $param = [
            'com' => self::getCompany($company),
            'num' => $num
        ];
        ...
        $result = Http::post('http://poll.kuaidi100.com/poll/query.do', $body);
        if ($result === false) {
        ...
        if (isset($result['returnCode']) && $result['returnCode']) {
            $this->errcode = (int)$result['returnCode'];
            $this->errmsg = $result['message'];
            return false;
        }
        if ($result['status'] != 200) {
            $this->errcode = (int)$result['status'];
            $this->errmsg = $result['message'];
            return false;
        }
        ...
        foreach ($result['data'] as $datum) {
            $details[] = [$datum['ftime'], $datum['context']];
        }
     ...
     * @return string
     */
    private static function getCompany($keyword)
    {
        $expresses = [
            //A
            '澳大利亚邮政'                    => 'auspost',    //英文结果
